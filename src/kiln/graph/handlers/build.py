...
_PRE_COMMIT_HOOK = """\
#!/bin/sh
# kiln scope enforcement — blocks commits touching out-of-scope files
SCOPE_FILE="$(git rev-parse --show-toplevel)/.kiln-scope"
[ -f "$SCOPE_FILE" ] || exit 0

STAGED=$(git diff --cached --name-only)
VIOLATIONS=""
for f in $STAGED; do
  [ "$f" = ".kiln-scope" ] && continue
  grep -qxF "$f" "$SCOPE_FILE" || VIOLATIONS="$VIOLATIONS  $f\\n"
done

if [ -n "$VIOLATIONS" ]; then
  printf "kiln: commit blocked — files outside allowed scope:\\n%s" "$VIOLATIONS" >&2
  printf "Allowed files listed in .kiln-scope\\n" >&2
  exit 1
fi
"""
...
def _setup_workspace(
    ...
    exclude_path.write_text(
        "# kiln: local excludes — not committed\n"
        "__pycache__/\n"
        "*.pyc\n"
        "*.pyo\n"
        ".coverage\n"
    ...
    if allowed_files:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"chore: set kiln scope for {branch}", "--no-verify"],
            capture_output=True,
            cwd=workspace,
        )
...
class BuildHandler:
    ...
    async def execute(self, node: GraphNode, config: Config) -> NodeResult:
        ...
        workspace = Path(tempfile.mkdtemp(prefix=f"kiln-{node.id}-"))
    ...
    def recover(self, node: GraphNode, config: Config) -> NodeResult | None:
        """If the node was running when kiln crashed, check if a PR already exists."""
