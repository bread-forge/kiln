...
class MergeHandler:
    ...
    async def _dispatch_repair(
        ...
        """Run a repair agent to fix CI failures on the PR branch. Returns cost_usd."""
...
Fix only what CI is complaining about. Do not refactor or expand scope."""
    ...
    async def _dispatch_conflict_resolution(
        ...
        """Dispatch an agent to rebase the branch onto main and resolve conflicts. Returns cost_usd."""
        from kiln.agents.runner import run_agent
...
You must preserve all intentional changes from the PR. Do not discard any features."""
    ...
    async def _dispatch_review_fix(
        ...
        """Dispatch an agent to address reviewer feedback on the PR. Returns cost_usd."""
        from kiln.agents.runner import run_agent
    ...
    async def execute(self, node: GraphNode, config: Config) -> NodeResult:
        ...
        if branch:
            ...
            if is_conflicting:
                ...
                if conflict_count < MAX_CONFLICT_ATTEMPTS:
                    ...
                    _c = await self._dispatch_conflict_resolution(pr_number, branch, repo, config)
                    if _c is not None:
                        _accumulated_cost += _c
                    ...
                    return NodeResult(
                        ...
                        cost_usd=_accumulated_cost or None,
                    )
                return NodeResult(
                    success=False,
        ...
        if branch and _has_changes_requested(pr_number, repo):
            ...
            if review_count < MAX_REVIEW_ATTEMPTS:
                ...
                _c = await self._dispatch_review_fix(
                    pr_number, branch, repo, config, review_comments
                )
                if _c is not None:
                    _accumulated_cost += _c
                await asyncio.sleep(CI_POLL_INTERVAL_SECONDS)
        ...
        if ci_status is False:
            ...
            if repair_count < MAX_REPAIR_ATTEMPTS and branch:
                ...
                _c = await self._dispatch_repair(pr_number, branch, repo, config)
                if _c is not None:
                    _accumulated_cost += _c
