...
def _check_bot_collaborator(repo: str, bot_token: str) -> CheckResult:
    ...
    if not bot_token:
        return CheckResult(
            "bot-collaborator",
            CheckStatus.FAIL,
            f"added yeast-bot to {repo} but KILN_GH_TOKEN is not set; cannot accept the invitation",
        )
...
def run_health_checks(repo: str) -> HealthReport:
    ...
    if os.environ.get("KILN_PROXY_SECRET"):
        ...
        checks.append(
            CheckResult(
                "proxy-secret",
                CheckStatus.WARN,
                "KILN_PROXY_SECRET not set — proxy will use an ephemeral per-session key",
            )
        )

    # 7. Not running inside Claude Code (nesting guard)
    # 5. KILN_GH_TOKEN present and valid (required for yeast-bot operations)
    bot_token = os.environ.get("KILN_GH_TOKEN") or ""
    if not bot_token:
        checks.append(
            CheckResult(
                "bot-token",
                CheckStatus.FAIL,
                "KILN_GH_TOKEN is not set. "
        ...
        if http_code == "200":
            checks.append(
                CheckResult("bot-token", CheckStatus.PASS, "KILN_GH_TOKEN is set and valid")
            )
        else:
            checks.append(
                CheckResult(
                    "bot-token",
                    CheckStatus.FAIL,
                    f"KILN_GH_TOKEN is set but invalid (HTTP {http_code}) — "
                    "regenerate yeast-bot's token and update KILN_GH_TOKEN.",
                )
            )
    ...
    if os.environ.get("KILN_AGENT") == "1":
        checks.append(
            CheckResult(
                "nesting-guard",
                CheckStatus.FAIL,
                "kiln orchestrator cannot run inside a kiln agent",
            )
        )
