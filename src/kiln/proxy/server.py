...
from kiln.proxy.token import ScopedToken, TokenError, issue_token, validate_token
...
class CredentialProxy:
    ...
    def __init__(self, secret: bytes | None = None) -> None:
        if secret is None:
            key = os.environ.get("KILN_PROXY_SECRET", "")
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._server.serve_forever, daemon=True, name="kiln-proxy"
        )
        self._thread.start()
