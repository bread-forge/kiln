...
@dataclass
class Config:
    """Runtime configuration for a kiln session."""
    ...
    max_retries: int = 3
    beads_dir: Path = field(default_factory=lambda: Path.home() / ".kiln" / "beads")
    ...
    def from_env(cls, repo: str) -> Config:
        return cls(
            ...
            beads_dir=Path(
                os.environ.get(
                    "KILN_BEADS_DIR",
                    str(Path.home() / ".kiln" / "beads"),
                )
            ),
            github_token=os.environ.get("KILN_GH_TOKEN") or None,
            research_backend=os.environ.get("KILN_RESEARCH_BACKEND", "anthropic"),
...
_REGISTRY_PATH = Path.home() / ".kiln" / "kiln.toml"
...
class Registry:
    """Platform repo registry backed by ~/.kiln/kiln.toml."""
